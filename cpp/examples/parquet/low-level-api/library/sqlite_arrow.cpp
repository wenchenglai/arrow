...
int load_data_to_arrow_v3_one_table_per_thread(
        ...
        std::string comma_separated_ids) {
    ...
    if (comma_separated_ids.empty()) {
        query += ";";
    } else {
        query += " WHERE defectKey$defectID IN (" + comma_separated_ids + ");";
    }

    //std::cout << "Final SQL Statement: " << query << std::endl;
...
table_ptr process_each_data_batch(
        ...
        bool has_encrypt,
        int reserve_size,
        std::vector<std::uint64_t> locator_keys) {
    ...
    if (memory_target == ArrowTablePerThread) {
        ...
        for (std::string file_path : file_paths) {
            // build the incremental ids, e.g. "1,2,3,4"
            std::string comma_separated_ids = "";

            //std::cout << "Before comma_separated_ids = " << comma_separated_ids << ", and file path: " << file_path << std::endl;

            if (!locator_keys.empty()) {
                comma_separated_ids = LocatorKey::GetIncrementalIds(locator_keys, file_path);
                //std::cout << "comma_separated_ids = " << comma_separated_ids << std::endl;
            }

            sum_num_rows_per_thread += load_data_to_arrow_v3_one_table_per_thread(
                            file_path,
                            int64_builder_map,
                            double_builder_map,
                            float_builder_map,
...
int SqliteArrow::record_batch_to_sqlite(std::shared_ptr<arrow::RecordBatch> record_batch, std::string dhl_name, std::string output_file_path) {
    ...
    if (SQLITE_OK != bResult) {
        std::cerr << "sqlite3_open_v2 failed: " << bResult << std::endl;

        if (nullptr != pDb) {
            std::cerr << "Error Message: " << sqlite3_errmsg(pDb) << std::endl;
            sqlite3_close_v2(pDb);
        }
        return EXIT_FAILURE;
    }
...
//
//        if (nullptr != pDb) {
//            std::cerr << "SQLite Error Message: " << sqlite3_errmsg(pDb) << std::endl;
//            sqlite3_close_v2(pDb);
//        }
//        return EXIT_FAILURE;
//    }
    ...
    if (SQLITE_OK != bResult) {
        std::cerr << "Cannot execute Create table if not exists command: " << bResult << std::endl;
...
//    upsert += "  WHERE excluded.validDate > phonebook2.validDate;";
    ...
    for (int64_t row_idx = 0; row_idx < record_batch->num_rows(); row_idx++) {
        ...
        for (auto&& field : fields) {
            ...
            if (arrow::Type::INT32 == col_type) {
                auto array = std::static_pointer_cast<Int32Array>(record_batch->GetColumnByName(col_name));
                if (array->length() > array_idx) {
                    sqlite3_bind_int(stmt, col_idx, array->Value(array_idx));
                }

            } else if (arrow::Type::INT64 == col_type) {
                auto array = std::static_pointer_cast<Int64Array>(record_batch->GetColumnByName(col_name));
                if (array->length() > array_idx) {
                    sqlite3_bind_int64(stmt, col_idx, array->Value(array_idx));
                }

            } else if (arrow::Type::FLOAT == col_type) {
                auto array = std::static_pointer_cast<FloatArray>(record_batch->GetColumnByName(col_name));
                if (array->length() > array_idx) {
                    sqlite3_bind_double(stmt, col_idx, array->Value(array_idx));
                }

            } else if (arrow::Type::DOUBLE == col_type) {
                auto array = std::static_pointer_cast<DoubleArray>(record_batch->GetColumnByName(col_name));
                if (array->length() > array_idx) {
                    sqlite3_bind_double(stmt, col_idx, array->Value(array_idx));
                }

            } else if (arrow::Type::BINARY == col_type) {
                auto array = std::static_pointer_cast<BinaryArray>(record_batch->GetColumnByName(col_name));
                if (array->length() > array_idx) {
                    int length;
                    const uint8_t* local_buffer = NULL;
                    local_buffer = array->GetValue(array_idx, &length);
